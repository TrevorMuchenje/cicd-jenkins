...
docker build -t trevormuchenje/cicd:latest . 
docker push trevormuchenje/cicd:latest

### running the container modelv1 using the trevormuchenje/cicd:latest image
docker run -d -it --name modelv1 -p 8005:8005 trevormuchenje/cicd:latest bash

### running the training script inside the container
docker exec modelv1 python prediction_model/training_pipeline.py

### running test script inside the container
docker exec modelv1 pytest -v --junitxml TestResults.xml --cache-clear

### copy test result from container to current directory
docker cp modelv1:/code/src/TestResults.xml .

### running the main.py file which starts the application
docker exec -d -w /code modelv1 python main.py

### use this to execute using a different port
